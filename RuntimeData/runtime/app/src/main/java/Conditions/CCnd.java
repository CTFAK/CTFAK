...
public abstract class CCnd extends CEvent
{
    ...
    public static CCnd create(CRunApp app)
    {
        ...
        if (cnd != null)
        {
            ...
            if (cnd instanceof CND_ONLOOP)
            {
                ...
                {
                    ((CND_ONLOOP)cnd).bDeterminedName = true;
                    ((CND_ONLOOP)cnd).name = ((EXP_STRING) pExp.tokens [0]).string.toLowerCase ();
                }
            }
			// *CND_ONLOOP*

			// *CND_EXTONLOOP*
            if (cnd instanceof CND_EXTONLOOP)
            {
	            CParamExpression pExp = (CParamExpression) cnd.evtParams[0];
                if (pExp.tokens.length == 2 && pExp.tokens[0].code == ((3 << 16) | 65535) && pExp.tokens[1].code==0)
                {
                    ((CND_EXTONLOOP)cnd).bDeterminedName = true;
                    ((CND_EXTONLOOP)cnd).name = ((EXP_STRING) pExp.tokens [0]).string.toLowerCase ();
                }
            }
			// *CND_EXTONLOOP*

			// *CND_ONEVENT*
            if (cnd instanceof CND_ONEVENT)
            {
	            CParamExpression pExp = (CParamExpression) cnd.evtParams[0];
                if (pExp.tokens.length == 2 && pExp.tokens[0].code == ((3 << 16) | 65535) && pExp.tokens[1].code==0)
                {
                    ((CND_ONEVENT)cnd).bDeterminedName = true;
                    ((CND_ONEVENT)cnd).name = ((EXP_STRING) pExp.tokens [0]).string.toLowerCase ();
                }
            }
			// *CND_ONEVENT*
    ...
    public boolean isColliding(CRun rhPtr)
    {
        ...
        ArrayList<CObject> list2 = null;	// = new ArrayList<CObject>();
        int index, n;
        CObject pHo2;
        do
        {
            ...
            {
				// Destroyed objects are no longer added to the list so don't need to scan the list anymore
				if ( list2 == null )
					list2 = list;
				else
					list2.addAll(list);
