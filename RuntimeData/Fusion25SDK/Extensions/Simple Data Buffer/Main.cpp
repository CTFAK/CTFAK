...
void LoadDataFile (LPRDATA rdPtr, LPCTSTR pFilename)
{
...
short WINAPI DLLExport ActLoadFile(LPRDATA rdPtr, long param1, long param2)
{
	...
	// Different file?
	if ( pFilename != NULL && _tcsicmp (rdPtr->sFilename, pFilename) != 0 )
	{
		// Load new file
...
short WINAPI DLLExport ActSaveFile(LPRDATA rdPtr, long param1, long param2)
{
	...
	// Different file?
	if ( pFilename != NULL && *pFilename != 0 && _tcsicmp (rdPtr->sFilename, pFilename) != 0 )
	{
		// Create file
		...
		if ( hf != HFILE_ERROR )
		{
			...
			// Copy filename
			_tcscpy(rdPtr->sFilename, pFilename);
